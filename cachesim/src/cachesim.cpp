...
//	cout << "oi A" << endl;
	Memory mem;
//	cout << "oi B0" << endl;
	Cache cache(16*1024,1,32, &mem); // Mudar
/*	
	// LOOP INTERCHANGE 
//	cout << "oi B" << endl;
...
/*	//BLOCKING BEFORE
	...
	for(int B=8;B<=8;B++){
		for (int jj = 0; jj < 100; jj = jj+B)
			for (int kk = 0; kk < 100; kk = kk+B)
				for (int i = 0; i < 100; i = i+1)
					for (int j = jj; j < min(jj+B,100); j = j+1)
					{
						cache.write(0,270000);
						for (int k = kk; k < min(kk+B,100); k = k+1) 
						{
							uint val = cache.read(i*100+k+300000);
							val += cache.read(k*100+j+600000);
							val += cache.read(270000);
